...
export default function MapView({ places, selectedCategories = [], className, onMarkerClick, favorites = [], onToggleFavorite, userTravellerCodes = [] }: Props) {
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({
      ...
      center: [9.0863, 45.8105], // Como centro
      zoom: 13
    });
    ...
    map.on('load', () => {
      ...
      // Aggiungi geocoder (ricerca) dopo il caricamento
      ...
      const directions = new MapboxDirections({
        ...
        language: 'it',
        interactive: true,
        placeholderOrigin: 'Scegli punto di partenza...',
        placeholderDestination: 'Scegli destinazione...'
      });
  ...
  useEffect(() => {
    ...
    // fit-to-bounds
    if (hasValidBounds && filtered.length > 0) {
