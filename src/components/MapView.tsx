...
export default function MapView({ places, selectedCategories = [], className, onMarkerClick, favorites = [], onToggleFavorite, userTravellerCodes = [], cities = [], selectedCity, onSelectCity, onDeselectCity }: Props) {
  ...
  const mapboxToken = 'pk.eyJ1IjoidGVvdGVvdGVvIiwiYSI6ImNta29kZ254bDA0dmszZXFuaGRjOW9uYnUifQ.NLn79JF3t7RsuGUDm9yhEQ';
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({
      ...
      center: [9.2000, 46.0000], // Centro Lago di Como
      zoom: 9.5,
      pitch: 0,
      bearing: 0
    });
  ...
  useEffect(() => {
    ...
    // TEMPORANEAMENTE DISABILITATO PER SCREENSHOT
    /* cities.forEach((city) => {
  ...
  return (
    <>
      <div className={className ?? "relative h-[70vh] w-full"}>
        {/* Search Bar - TEMPORANEAMENTE NASCOSTO PER SCREENSHOT */}
        {false && selectedCity && onDeselectCity && (
        ...
        {/* Movie Mood Button - TEMPORANEAMENTE NASCOSTO PER SCREENSHOT */}
        {false && selectedCity && (
