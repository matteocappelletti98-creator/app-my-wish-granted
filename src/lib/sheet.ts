export type Place = { name: string; city: string; country: string; description: string; image?: string; status: string; };
...
export async function fetchPlacesFromSheet(csvUrl: string): Promise<Place[]> {
  ...
  return rows.slice(1).map(r => {
    const rec: any = {}; headers.forEach((h, i) => rec[h] = (r[i] ?? "").trim());
    return {
      name: rec.name || "", city: rec.city || "", country: rec.country || "",
      description: rec.description || "", image: rec.image || "", status: (rec.status || "").toLowerCase(),
    } as Place;
  });
}